...
public abstract partial class SharedVisualBodySystem
{
    ...
    private void ApplyAppearanceTo(Entity<VisualBodyComponent?> ent, HumanoidCharacterAppearance appearance, Sex sex)
    {
        ...
        // Aurora's Song - Apply scaling (height and width)
        if (appearance.Height != 1.0f || appearance.Width != 1.0f)
        {
            var scaleVisuals = EnsureComp<ScaleVisualsComponent>(ent);
            var appearanceComp = EnsureComp<AppearanceComponent>(ent);
            _appearance.SetData(ent, ScaleVisuals.Scale, new Vector2(appearance.Width, appearance.Height), appearanceComp);
        }
    }
